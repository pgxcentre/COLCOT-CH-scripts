...
def mutect2_tumor_only(inputTumor, tumor_name, outputVCF, panel_of_normals=None, intervals=[], exclude_intervals=[], interval_list=None):
    return Job(
		[inputTumor, interval_list],
		[outputVCF],
		[
			['gatk_mutect2', 'module_java'],
			['gatk_mutect2', 'module_gatk']
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --tumor-sample {tumor_name} \\
  --germline-resource {known_sites} \\
  --output {outputVCF}{pon}{interval_list}{exclude_intervals}""".format(
			...
			tumor_name=tumor_name,
			outputVCF=outputVCF,
			pon=" \\\n -pon " + panel_of_normals if panel_of_normals else "",
			interval_list=" \\\n  --interval-padding 100 --intervals " + interval_list if interval_list else "",
			exclude_intervals="".join(
				" \\\n  --exclude-intervals " + exclude_interval for exclude_interval in exclude_intervals)
		)
...
def get_pileup_summaries(input_bam, output, interval=None):
	return Job(
		[input_bam],
		[output],
		[
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}{intervals}""".format(
			...
			options=" " + config.param('gatk_get_pileup_summaries', 'options') if config.param('gatk_get_pileup_summaries', 'options') else "",
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --output {output}""".format(
			...
			options=" " + config.param('gatk_calculate_contamination', 'options') if config.param('gatk_calculate_contamination', 'options') else "",
			input=input,
...
def filter_mutect_calls(variants, output, contamination=None, stats=None):
		return Job(
			[variants],
			[output],
			[
...
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  ...
  --variant {variants} \\
  {contamination_table} \\
  --reference {reference_sequence}{stats} \\
  --output {output}""".format(
				...
				options=" " + config.param('gatk_filter_mutect_calls', 'options') if config.param('gatk_filter_mutect_calls', 'options') else "",
				variants=variants,
                reference_sequence=config.param('gatk_mutect2', 'genome_fasta', type='filepath'),
...
def filter_by_orientation_bias(variants, output, sequencing_artifacts):
		return Job(
			[variants],
			[output],
			[
				['gatk_filter_by_orientation_bias', 'module_java'],
				['gatk_filter_by_orientation_bias', 'module_gatk']
			],
			command="""\
gatk --java-options "-Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram}" \\
  FilterByOrientationBias{options} \\
  --variant {variants} \\
  -P {sequencing_artifacts} \\
  -R {reference_sequence} \\
  -AM G/T \\
  -AM C/T \\
  --output {output}""".format(
				tmp_dir=config.param('gatk_filter_by_orientation_bias', 'tmp_dir'),
				java_other_options=config.param('gatk_filter_by_orientation_bias', 'java_other_options'),
				ram=config.param('gatk_filter_by_orientation_bias', 'ram'),
				options=" " + config.param('gatk_filter_by_orientation_bias', 'options') if config.param('gatk_filter_by_orientation_bias', 'options') else "",
				variants=variants,
				reference_sequence=config.param('gatk_filter_by_orientation_bias', 'genome_fasta', type='filepath'),
				sequencing_artifacts=sequencing_artifacts,
				output=output
			)
		)
