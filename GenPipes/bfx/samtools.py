...
def sort_pipe(input_bam, output_bam):
    ...
    return Job(
        ...
        [['samtools_sort', 'module_samtools']],
        command="""\
samtools sort {threads}{other_options} \\
  {output_prefix}{input_bam}""".format(
        threads="--threads " + config.param('samtools_sort', 'threads', required=False) if config.param('samtools_sort', 'threads', required=False) else "",
        other_options= " " + config.param('samtools_sort', 'other_options', required=False) if config.param('samtools_sort', 'other_options', required=False) else "",
        input_bam=input_bam,
        output_prefix=" -o " + output_bam + " " if output_bam else ""
        ),
        removable_files=[output_bam]
    )
...
def depth(input, output=None, target=None, options="", removable=True):
    return Job(
        [input, target],
        [output],
        [
            ['samtools_depth', 'module_samtools']
        ],
        command="""\
samtools depth {options} \\
  {target}{output} {input}""".format(
        options=options,
        input=input,
        target=" -b " + target if target else "",
