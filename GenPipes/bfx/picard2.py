...
# Convert fastq file to SAM/BAM format
def fastq_to_sam(fastq, second_end_fastq, output, sample_name=None, read_group=None, library=None, platform=None, run=None, center=None):
    return Job(
            [fastq, second_end_fastq],
            [output],
            [
                ['picard_fastq_to_sam', 'module_java'],
                ['picard_fastq_to_sam', 'module_picard']
            ],
            command="""\
java -Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram} -jar $PICARD_HOME/picard.jar FastqToSam \\
 VALIDATION_STRINGENCY=LENIENT \\
 F1={fastq}{second_end_fastq}{output_file}{read_group}{sample_name}{library}{platform}{run}""".format(
            tmp_dir=config.param('picard_fastq_to_sam', 'tmp_dir'),
            java_other_options=config.param('picard_fastq_to_sam', 'java_other_options'),
            ram=config.param('picard_fastq_to_sam', 'ram'),
            fastq=fastq,
            second_end_fastq=" \\\n  F2=" + second_end_fastq if second_end_fastq else "",
            output_file=" \\\n O=" + output if output else "",
            read_group=" \\\n  RG=" + read_group if read_group else "",
            sample_name=" \\\n  SM=" + sample_name if sample_name else "",
            library=" \\\n  LB=" + library if library else "",
            platform=" \\\n  PL=" + platform if platform else "",
            center=" \\\n  CN=" + center if center else "", 
            run=" \\\n  PU=run" + run if run else "",
            ),
            removable_files=[output]
        )
# Merge alignment data from a SAM or BAM with data in an unmapped BAM file.
def merge_alignments(aligned, unmapped, output):
    inputs=[aligned, unmapped]
    if not isinstance(inputs, list):
        inputs=[inputs]
    return Job(
            inputs,
            ...
            command="""\
java -Djava.io.tmpdir={tmp_dir} {java_other_options} -Xmx{ram} -jar $PICARD_HOME/picard.jar MergeBamAlignment \\
 VALIDATION_STRINGENCY=SILENT CREATE_INDEX=true \\
 TMP_DIR={tmp_dir} \\
 ALIGNED={aligned} \\
 UNMAPPED={unmapped} \\
 ...
 R={reference}""".format(
            ...
            unmapped=unmapped,
            output=output,
            reference=config.param('picard_merge_sam_files', 'genome_fasta', type='filepath'),
            ),
            removable_files=[output, re.sub("\.([sb])am$", ".\\1ai", output)]
        )

def collect_hs_metrics(input, output, intervals, reference_sequence=None):
