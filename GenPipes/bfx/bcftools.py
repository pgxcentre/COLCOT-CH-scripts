...
def split_vep(input, output, filter_options):
    """
    extract and split vep output
    """
    return Job(
        [input],
        [output],
        [
            ['bcftools_split_vep', 'module_bcftools']
        ],
        command="""(
echo -n -e "CHROM\\tPOS\\tREF\\tALT\\tFILTER\\tSAMPLE\\tGT\\tDP\\tVD\\tAD\\tAF\\tRD\\tALD\\t"
bcftools +split-vep -l {input} | cut -f2 | tr '\\n' '\\t' \\
        | sed -e 's/\\t$/\\n/'
bcftools +split-vep {filter_options} \\
  {input}{output}""".format(
        input=" \\\n " + input if input else "",
        filter_options=filter_options,
        output=" \\\n ) > " + output if output else ""
