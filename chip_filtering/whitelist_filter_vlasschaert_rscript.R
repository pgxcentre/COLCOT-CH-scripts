...
ressource_dir <- args[1]

gList <- fread(paste(ressource_dir, "NEJM_2017_genes_01262020.txt", sep = "/"))
whitelist.mis <- fread(paste(ressource_dir, "CHIP_missense_vars_agb_01262020.txt", sep = "/"))
whitelist.splice <- fread(paste(ressource_dir, "CHIP_splice_vars_agb_01262020.txt", sep = "/"))
whitelist.LoF <- fread(paste(ressource_dir, "CHIP_nonsense_FS_vars_agb_01262020.txt", sep = "/"))

input_filename <- args[2]
out_dir <- args[3]
sample_id <- args[4]
# vars1<-fread(paste(sample_id, ".annovar_out.hg38_multianno.txt", sep = ""))
vars1<-fread(input_filename)
...
write.csv(varsOI.func,paste(out_dir, "/", sample_id, ".annovar.varsOI.allvariants.csv", sep=""), row.names=FALSE)
write.csv(varsOI.func[whitelist==T,],paste(out_dir, "/", sample_id, ".annovar.varsOI.wl.csv", sep=""), row.names=FALSE)
